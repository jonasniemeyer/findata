...
def test_logo_missing():
    assert YahooReader("ESCA").logo() == b"\n"
...
class TestETF:
    ...
    def test_fund_statistics(self):
        ...
        assert all(
            key in statistics.keys() for key in (
                "company",
                "type",
                "expense_ratio",
                "turnover",
    ...
    def test_holdings(self):
        ...
        assert all(
            key in holdings.keys() for key in (
                "equity_share",
                "bond_share",
                "holdings",
                "equity_data",
        ...
        assert all(
            key in holdings["equity_data"] for key in (
        ...
        assert all(
            key in holdings["bond_data"] for key in (
                "average_maturity",
                "average_duration",
            )
        )
        assert all(
            key in holdings["bond_ratings"] for key in (
                "bb",
                "aa",
                "aaa",
                "a",
        ...
        assert all(
            key in holdings["sector_weights"] for key in (
                "real_estate",
                "consumer_cyclical",
                "basic_materials",
                "consumer_defensive",
